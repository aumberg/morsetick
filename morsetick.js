(function() {
	if (window.morsetick && window.morsetick["off"]) {
		...
		if (console && ("function" === typeof console.log)) {
			console.log("can't start morsetick");
		}
	...
	m["soundcloud search and play link"] = function(request) {
		...
		$.getJSON("http://api.soundcloud.com/tracks/", param)
			.done(
				function(traks) {
					if (traks.length) {
	...
	m["audio play"] = function(stringSrc) {
		var audio = m["memory audio"];
		var volume = m["option audio volume"];

		if (!audio || !volume || !stringSrc) {
			return;
		}
	...
	m["audio source"] = function() {
		return m["memory audio"].src;
	};

	m["audio stop"] = function() {
		m["memory audio"].pause();
	};
	...
	m["green"] = function() {
		if (arguments.length && console && ("function" === typeof console.log)) {
			console.log("morsetick " + m["option program version"] + ", LOG: ", arguments);
		}
	};

	m["red"] = function() {
		if (arguments.length && console && ("function" === typeof console.error)) {
